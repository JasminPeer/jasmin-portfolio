    ...
    <section id="contact" className="section-pad bg-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto" ref={ref}>
        ...
        >
          ...
          <p className="text-base text-slate-500 mt-3 font-light max-w-md">
            I'm actively looking for entry-level software roles. Let's talk!
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            {contacts.map(({ icon, label, val, href }, i) => (
              ...
              >
                {href ? (
                  <a href={href} className="card-base p-5 flex items-center gap-4 group hover:border-violet-200">
                    <div className="w-10 h-10 rounded-xl bg-violet-50 flex items-center justify-center text-lg group-hover:scale-110 transition-transform">
                      {icon}
                    </div>
                    <div>
                      <p className="text-xs text-slate-400 font-medium">{label}</p>
                      <p className="text-sm font-semibold text-slate-800 group-hover:text-violet-700 break-all">
                        {val}
                      </p>
                    </div>
                  </a>
            ...
            >
              <a href={data.github} target="_blank" rel="noreferrer" className="btn-outline text-xs">GitHub</a>
              <a href={data.linkedin} target="_blank" rel="noreferrer" className="btn-outline text-xs">LinkedIn</a>
            </motion.div>
          </div>
          ...
          >
            {sent ? (
              <div className="card-base p-10 flex flex-col items-center text-center gap-4">
                ...
                <button onClick={() => setSent(false)} className="btn-outline text-xs">
                  Send another
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="card-base p-8 flex flex-col gap-5">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="btn-dark self-start disabled:opacity-70"
                >
